...
async function convertCurrency() {
    ...
    try {
        ...
        exchangeRateDisplay.textContent = `1 ${currencyFrom} = ${rate} ${currencyTo}`;
    } catch (error) {
        console.error('Error fetching exchange rates:', error);
        exchangeRateDisplay.textContent = ''; // Clear exchange rate display on error
    }
}
...
async function reverseConvertCurrency() {
    ...
    if (!amountTo) {
        amountFrom.value = '';
        exchangeRateDisplay.textContent = ''; // Clear exchange rate display if no amount is entered
        return;
    }
    ...
    try {
        ...
        exchangeRateDisplay.textContent = `1 ${currencyFrom} = ${await fetchExchangeRate(currencyFrom, currencyTo)} ${currencyTo}`; // Always show rate from currency-from
    } catch (error) {
        console.error('Error fetching exchange rates:', error);
        exchangeRateDisplay.textContent = ''; // Clear exchange rate display on error
    }
